...
if(typeof wmvplayer == "undefined") {
	var wmvplayer = new Object();
	wmvplayer.utils = new Object();
}
...
****************************************************************************/
wmvplayer.Player = function(cnt,src,cfg) {
	this.controller;
	this.model;
	this.view;
	this.configuration = {
...
wmvplayer.Player.prototype = {
	...
	onLoadHandler: function(pid,tgt,sdr) {
		...
		tgt.controller = new wmvplayer.Controller(tgt.configuration);
		tgt.view = new wmvplayer.View(tgt.configuration,tgt.controller);
		tgt.model = new wmvplayer.Model(tgt.configuration,tgt.controller,tgt.view);
...
****************************************************************************/
wmvplayer.Controller = function(cfg) {
	this.configuration = cfg;
}

wmvplayer.Controller.prototype = {
	...
	setState: function(old,stt) {
		...
		if(old == 'Closed' && pos > 0) {
			setTimeout(wmvplayer.utils.delegate(this,this.setScrub),200,pos);
		} 
	},
	...
	setLoad: function(fil) {
		...
		if(this.configuration['autostart'] == 'true') {
			setTimeout(wmvplayer.utils.delegate(this.model,this.model.goStart),100);
		}
	},
	...
	setFullscreen: function() {
		...
		wmvplayer.utils.delegate(this.view,this.view.onFullscreen);
	}
}
...
****************************************************************************/
wmvplayer.View = function(cfg,ctr) {
	this.configuration = cfg;
	...
	this.configuration['sender'].getHost().content.onResize = 
		wmvplayer.utils.delegate(this,this.resizePlayer);
	this.configuration['sender'].getHost().content.onFullScreenChange = 
		wmvplayer.utils.delegate(this,this.onFullscreen);
	this.assignColorsClicks();
	this.resizePlayer();
}

wmvplayer.View.prototype = {
	...
	onFullscreen: function(fss) {
		...
		if(fst) { 
			this.fstimeout = setTimeout(wmvplayer.utils.delegate(this,
				this.hideFSControls),2000);
			this.fslistener = this.display.addEventListener('MouseMove',
				wmvplayer.utils.delegate(this,this.showFSControls));
	...
	onTime: function(elp,dur) {
		...
		if(dur > 0) {
			...
			snd.findName("ElapsedText").Text = wmvplayer.utils.timestring(elp);
			snd.findName("RemainingText").Text = wmvplayer.utils.timestring(dur-elp);
	...
	assignColorsClicks: function() {
		...
		if(this.configuration['linkfromdisplay'] == 'false') { 
			this.display.addEventListener('MouseLeftButtonUp',
				wmvplayer.utils.delegate(this.controller,
				this.controller.setPlay));
		} else { 
			this.display.addEventListener('MouseLeftButtonUp',
				wmvplayer.utils.delegate(this.controller,
	...
	assignButton: function(btn,act) {
		...
		el1.addEventListener('MouseLeftButtonUp',
			wmvplayer.utils.delegate(this.controller,act));
		el1.addEventListener('MouseEnter',
			wmvplayer.utils.delegate(this,this.rollOver));
		el1.addEventListener('MouseLeave',
			wmvplayer.utils.delegate(this,this.rollOut));
	...
	assignSlider: function(sld,act) {
		...
		el1.Cursor = "Hand";
		el1.addEventListener('MouseLeftButtonUp',
			wmvplayer.utils.delegate(this,act));
		el1.addEventListener('MouseEnter',
			wmvplayer.utils.delegate(this,this.rollOver));
		el1.addEventListener('MouseLeave',
			wmvplayer.utils.delegate(this,this.rollOut));
...
****************************************************************************/
wmvplayer.Model = function(cfg,ctr,vie) {
	this.configuration = cfg;
	this.controller = ctr;
	this.view = vie;
	...
	this.video.BufferingTime = 
		wmvplayer.utils.spanstring(this.configuration['bufferlength']);
	this.video.AutoPlay = true;
	this.video.AddEventListener("CurrentStateChanged",
		wmvplayer.utils.delegate(this,this.stateChanged));
	this.video.AddEventListener("MediaEnded",
		wmvplayer.utils.delegate(this,this.mediaEnded));
	this.video.AddEventListener("BufferingProgressChanged",
		wmvplayer.utils.delegate(this,this.bufferChanged));
	this.video.AddEventListener("DownloadProgressChanged",
		wmvplayer.utils.delegate(this,this.downloadChanged));
	if(this.configuration['image'] != '') {
		this.preview.Source = this.configuration['image'];
	}
}

wmvplayer.Model.prototype = {
	goPause: function(sec) {
		this.video.pause();
		if(!isNaN(sec)) {
			this.video.Position = wmvplayer.utils.spanstring(sec);
		}
		this.timeChanged();
	},
	...
	stateChanged: function() {
		...
		if(stt != this.state) {
			...
			if(stt != "Playing" && stt != "Buffering" && stt != "Opening") {
				...
				this.timeint = setInterval(wmvplayer.utils.delegate(
...
****************************************************************************/
wmvplayer.utils.delegate = function(obj,fcn) {
	return function() {
		return fcn.apply(obj,arguments);
	}
}
wmvplayer.utils.timestring = function(stp) {
...
wmvplayer.utils.spanstring = function(stp) {
	var hrs = Math.floor(stp/3600);
	var min = Math.floor(stp%3600/60);
