...
<h1 style="padding:8px;">popular on friendfeed in past 48 hours</h1>
...
<ul id="public-feed-holder">
	...
	<? for ($i=0; $i < count($data) ; $i++) { 
		...
		<li class="public-feed-item" >
			...
			<div class="feed-item-main">
				...
				<div style="height: 21px;">
					...
					<a href="<?= $item_link ?>" id="feed-play-button" style="float:right;"></a>
				...
				<div class="feed-sub-title">
					...
					<div class="feed-sub-title-detail">
									...
									found on <a class="blue-link-small" href="http://friendfeed.com">
										friendfeed
									</a> <?= $data[$i]['vCount'] ?> time<?=  ($data[$i]['vCount'] > 1) ? ("s") : ("")  ?> 
									
									<?
										if ( $data[$i]['likes'] > 0 ) {
											echo " with " . $data[$i]['likes'] . " like";
											echo ($data[$i]['likes'] > 1) ? ("s") : ("");
										}
									
									?>
									
									via <a class="blue-link-small" href="http://youtube.com/watch?v=<?= $data[$i]['youtube_id'] ?>" target="_new">youtube</a>
						
						
									<? if (User::$user_su == '1') { ?>	
								&nbsp;&nbsp;&nbsp;&nbsp;<a style="color: #cc4400;" href="/public/add_ext/<?= $data[$i]['youtube_id'] ?>?type=101&comment=found <?= $data[$i]['vCount'] ?> times with <?= $data[$i]['likes'] ?> likes and <?= $data[$i]['comments'] ?> comments ">add to public</a>
								<? } ?>
				...
				<div class="feed-comment" style="border-left: 5px solid #fff; margin-left: 3px; margin-top: 10px; padding-left: 4px; " >
		...
		<div id="name">
		<img src="<?= $data[$i]['thumb']?>" style="float:left; padding: 5px"/>	

		<a href="/videos/detail/<?= $data[$i]['video_id']?>"><?= $data[$i]['title']?></a><br/>
		from: <a href="/shows/detail/<?= $data[$i]['show_id']?>"><?= $data[$i]['show_title']?></a><br/>
		<?= date("m-d-y",strtotime($data[$i]['date_added'])) ?><br/>
		<?= $data[$i]['type']?><br/>
		<?= $data[$i]['comment']?><br/>
					<? if (User::$user_su == '1') { ?>	
				<a href="/public/item_delete/<?= $data[$i]['public_id']?>">delete</a>
				<? } ?>
		<br clear="both"/>
		</div>
	...
	<div id="right-column" style="" >
		...
		<? } ?>
			...
			<div class="right-column-box-light" style="margin-top: 30px;">
				...
				<div style="border-bottom: 1px dotted #d6d2ad; padding: 8px 0px 8px 0px; margin: 0px 15px 0px 15px; font-weight: bold; font-size: 13px;">
